...
VALUES
    ...
    ('Jane', 'Smith', 'jane.smith', '$2a$10$iN0lUG1/ekAPik1AIA8hmu41akz/3riRyGIbvlxGt58NR7NUVjQce', true),
...
VALUES
    ((SELECT id FROM users WHERE username = 'jane.smith'), (SELECT id FROM training_types WHERE training_type_name = 'Cardio')),
...
VALUES
    ((SELECT id FROM trainees WHERE user_id = (SELECT id FROM users WHERE username = 'john.doe')),
     (SELECT id FROM trainers WHERE user_id = (SELECT id FROM users WHERE username = 'jane.smith')),
